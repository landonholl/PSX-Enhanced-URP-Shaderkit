...
namespace PSXShaderKit
{
    ...
    public class PSXShaderManager : MonoBehaviour
    {
        ...
        [Header("PSX Fog")]
        [SerializeField]
        private bool _EnablePSXFog = false;
        [SerializeField]
        [Tooltip("Distance at which fog begins.")]
        private float _FogStart = 5.0f;
        [SerializeField]
        [Tooltip("Distance at which fog is fully opaque.")]
        private float _FogEnd = 20.0f;
        [SerializeField]
        private Color _FogColor = new Color(0.1f, 0.1f, 0.1f, 1.0f);
        [SerializeField]
        [Tooltip("Number of discrete fog bands (0 = smooth linear). 4-8 gives a PSX-style stepped look.")]
        private float _FogSteps = 0.0f;

        [Header("Film Grain")]
        [SerializeField]
        [Range(0f, 0.05f)]
        private float _GrainStrength = 0.0f;
        [SerializeField]
        [Range(1f, 8f)]
        private float _GrainSize = 1f;
        ...
        void Update()
        {
            // Grain needs Time.time so must update every frame
            Shader.SetGlobalFloat("_PSX_NoiseTime", Time.time);
        }

        public void UpdateValues()
        {
            Shader.SetGlobalFloat("_PSX_GrainStrength", _GrainStrength);
            Shader.SetGlobalFloat("_PSX_GrainSize",     _GrainSize);
            Shader.SetGlobalFloat("_PSX_NoiseStrength", _NoiseStrength);
            Shader.SetGlobalFloat("_PSX_FogEnabled", _EnablePSXFog ? 1.0f : 0.0f);
            Shader.SetGlobalFloat("_PSX_FogStart",   _FogStart);
            Shader.SetGlobalFloat("_PSX_FogEnd",     _FogEnd);
            Shader.SetGlobalColor("_PSX_FogColor",   _FogColor);
            Shader.SetGlobalFloat("_PSX_FogSteps",   _FogSteps);
