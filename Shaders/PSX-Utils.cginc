...
{
    // round() instead of floor() so UV-seam duplicate vertices (same position, different UV)
    // and FBX-imported vertices with sub-epsilon position differences snap to the same grid cell.
    // Visually identical PSX wobble; eliminates crack seams along shared edges.
    return _PSX_GridSize < 0.00001f ? vertex : (round(vertex * _PSX_GridSize) / _PSX_GridSize);
}
...
{
	// Bayer 4x4 ordered dither matrix, values 0-7 (average = 3.5, unbiased).
	// Derived from standard Bayer 4x4 (0-15) divided by 2.
	// Previous version used -4..+3 which averaged to -0.5, causing systematic darkening.
